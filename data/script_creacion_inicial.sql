...
SELECT a.usu_Id,YEAR(c.comp_Fecha)
FROM J2LA.Usuarios a, J2LA.Publicaciones b, J2LA.Compras c
WHERE a.usu_Id = b.pub_usu_Id
AND b.pub_Codigo = c.comp_pub_Codigo
AND c.comp_cal_Codigo IS NOT NULL
GROUP BY a.usu_Id,YEAR(c.comp_Fecha)

DECLARE cursor_usuario CURSOR for (select usucal_usu_Id,usucal_Anio from J2LA.Usuarios_Calificaciones)
declare @usuario int,@anio int
open cursor_usuario
fetch next from cursor_usuario into @usuario,@anio
while @@FETCH_STATUS = 0
	begin
		update J2LA.Usuarios_Calificaciones
		set usucal_Puntos_Primero = ISNULL((select SUM(Ca.cal_Cant_Estrellas)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (1,2,3)),0),
			usucal_Cant_Ventas_Primero = ISNULL((select COUNT(*)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (1,2,3)),0),
			usucal_Puntos_Segundo = ISNULL((select SUM(Ca.cal_Cant_Estrellas)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (4,5,6)),0),
			usucal_Cant_Ventas_Segundo = ISNULL((select COUNT(*)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (4,5,6)),0),
			usucal_Puntos_Tercero = ISNULL((select SUM(Ca.cal_Cant_Estrellas)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (7,8,9)),0),
			usucal_Cant_Ventas_Tercero = ISNULL((select COUNT(*)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (7,8,9)),0),
			usucal_Puntos_Cuarto = ISNULL((select SUM(Ca.cal_Cant_Estrellas)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (10,11,12)),0),
			usucal_Cant_Ventas_Cuarto = ISNULL((select COUNT(*)
									from J2LA.Publicaciones P,J2LA.Compras Co,J2LA.Calificaciones Ca
									where @usuario = P.pub_usu_Id
									and P.pub_Codigo = Co.comp_pub_Codigo
									and Co.comp_cal_Codigo = Ca.cal_Codigo
									and YEAR(Co.comp_Fecha)=@anio
									and MONTH(Co.comp_Fecha) IN (10,11,12)),0)
		where usucal_usu_Id = @usuario
		and usucal_Anio = @anio
		fetch next from cursor_usuario into @usuario,@anio
	end
